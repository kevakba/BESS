...
while result_df.empty:
    try:
        ...
        result_df['solar_generation'] = df[df['Forecast Transaction Date'].apply(lambda x: x.split(' ')[0]) == f'{date_}']['Most Likely']
...
result_df.to_csv(f'/home/kevin/Downloads/BESS/data/raw/forecast/solar_generation_{str(date_).replace("-", "")}_{str(date_).replace("-", "")}.csv')